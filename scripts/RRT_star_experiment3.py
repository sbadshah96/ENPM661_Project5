...
def check_line_obstacles(node1,node2):
    ...
    for i in range(1,int(find_distance(node1[0],node1[1],node2[0],node2[1])+1),1):
...
def find_parent(node):
    ...
    if closest_node != None and closest_node != node:
        val = check_line_obstacles(node,closest_node)
        if val == True :
            cost = find_distance(node[0],node[1],closest_node[0],closest_node[1])
            total_cost = cost + node_records[str(closest_node)][1]
            parent_node = closest_node
            return parent_node,total_cost
        else:
            return None, None
    else:
        return None,None
...
def viz():
    pygame.init()
    video = vidmaker.Video("rrt_star_experiment3_"+str(iterations)+"_"+str(check_radius_RRTS)+".mp4", late_export=True)
    size = [600, 200]
